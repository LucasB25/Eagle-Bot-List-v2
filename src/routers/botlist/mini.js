...
console.log("[localhost]: Botlist/Mini router loaded.");
...
app.get("/search", async (req, res) => {
  ...
  if (req.query.q.length < 1)
    return res.redirect(
      `/error?code=404&message=You didn't specify any search`
    );
  let data = x.filter(
    (a) => a.status == "Approved" && a.username.includes(req.query.q)
  );
  if (page < 1) return res.redirect(`/bots`);
  if (data.length <= 0) return res.redirect("/");
  if (page > Math.ceil(data.length / 6)) return res.redirect(`/bots`);
...
app.get("/tags", async (req, res) => {
  ...
  function tagsAmount(tag) {
    const amount = x.filter((bot) => bot.tags.includes(tag));

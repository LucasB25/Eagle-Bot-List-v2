...
console.log("[localhost/servers]: Edit router loaded.");
...
app.post("/:guildID/edit", global.checkAuth, async (req, res) => {
  ...
  if (autoCreate === "true") {
    guild.invites.fetch().then(async (fetchinvite) => {
      fetchinvite.find((a) => a.inviter.id === client.user.id)
        ? fetchinvite.find((a) => a.inviter.id === client.user.id).code
        : await guild.channels.cache.random().createInvite({ maxAge: 0 });
    });
    guild.invites.fetch().then(async (fetchinvite) => {
      let inviteURL = fetchinvite.find(
        (a) => a.inviter.id === client.user.id
      ).url;
      await sdata.findOneAndUpdate(
        {
          id: req.params.guildID,
